...
Gaps:
- Only one live Docker server implementation family is covered: Samba.
...
- The harness artifacts now record external durable-handle v2 outcome or skip data, but checked-in pass evidence is still backlog.
- Advanced SMB 3.x outcome reporting is now present in the external harness artifacts, but checked-in pass evidence is still backlog.
...
- There is no checked-in passing Linux kernel CIFS client evidence yet.
...
- `eng/run-linux-cifs-interop.ps1` provides a Linux kernel CIFS client harness, but no passing evidence is checked in yet.
...
- The external Python, Samba, and Windows harness artifacts can now record structured advanced SMB 3.x durable-handle v2 or skip data, but the matrices still lack checked-in external pass evidence for that behavior.
...
Impact:
- Advanced reconnect, lease, and modern SMB 3.x behavior is not yet externally evidenced end to end even though the artifact/reporting path now exists.
...
| TG-003 | P0 | in_progress | Put external interop into recurring CI. `.github/workflows/external-interop.yaml` now provides scheduled and manual execution on a self-hosted Windows runner labeled `external-interop`, runs `eng/run-integration-gates.ps1`, optionally runs SMB 3.1.1 preview lanes, forwards optional `OPENCIFS_ENABLE_CROSS_VERSION_INTEROP` plus previous-package version settings for the cross-version managed lane, forwards optional `OPENCIFS_DFS_*` configuration for the external DFS namespace lane, forwards optional `OPENCIFS_ENABLE_LINUX_CIFS_INTEROP` plus `OPENCIFS_LINUX_CIFS_IMAGE_NAME` for the privileged Linux CIFS lane, forwards optional `OPENCIFS_WINDOWS_SERVER_*` configuration for the live Windows-server lane, and uploads interop artifacts including `artifacts/cross-version-managed-interop/**`, `artifacts/dfs-interop/**`, `artifacts/windows-server-interop/**`, and `artifacts/linux-cifs-interop/**`. | Complete when the repository has a provisioned runner with Docker, Python, Windows SMB client support, and scheduled runs producing current artifacts that can refresh the matrix dates. | Added `.github/workflows/external-interop.yaml`; it now forwards optional cross-version, DFS, Linux-CIFS, and Windows-server settings and uploads those artifacts, but still needs runner provisioning and first successful scheduled evidence refresh. |
| TG-004 | P1 | in_progress | Add Linux kernel CIFS client coverage against `Sample.OpenCifsServer`. `eng/run-linux-cifs-interop.ps1` now builds a Dockerized `cifs-utils` image, starts `Sample.OpenCifsServer`, and uses privileged `mount.cifs` lanes for SMB 2.0.2, SMB 2.1, and SMB 3.0.2 to exercise mount, create, read, write, large-file copy, rename, non-empty-directory delete rejection, and cleanup. `eng/run-integration-gates.ps1` now auto-invokes that lane when `OPENCIFS_ENABLE_LINUX_CIFS_INTEROP=true`, so privileged hosts can include the Linux kernel CIFS matrix in recurring interop runs while normal hosts still skip it safely. | Complete when the harness has passing evidence on a Docker host that supports privileged CIFS mounts, `docs/interop-matrix.md` has a `Sample.OpenCifsServer | Linux kernel CIFS client` pass row with a current date, and the artifact records kernel, cifs-utils, mount options, dialect, and operation results. Byte-range lock-conflict coverage can be added after the mount path is stable. | Added `eng/docker/linux-cifs-interop/Dockerfile`, `eng/run-linux-cifs-interop.ps1`, wired the lane into `eng/run-integration-gates.ps1`, and documented the opt-in environment path in `README.md`; still needs privileged-runner evidence refresh. |
...
| TG-006 | P1 | in_progress | Add managed client/server cross-version compatibility coverage. `eng/run-cross-version-managed-interop.ps1` now restores released `OpenCIFS.Client` and `OpenCIFS.Server` packages from a configurable package source, generates temporary current-project and previous-package consumers, runs `current client -> previous server` plus `previous client -> current server` across SMB 2.0.2, SMB 2.1, and SMB 3.0.2, and writes `artifacts/cross-version-managed-interop/cross-version-managed-interop.json`. `eng/run-integration-gates.ps1` now auto-invokes that lane when `OPENCIFS_ENABLE_CROSS_VERSION_INTEROP=true`, and `docs/interop-matrix.md` records both cross-version directions as harness-ready. | A cross-version compatibility artifact exists with package or commit versions; `docs/interop-matrix.md` records N-to-N-1 coverage; release notes can cite explicit compatibility evidence. | Added `eng/run-cross-version-managed-interop.ps1`, wired it into `eng/run-integration-gates.ps1`, documented it in `README.md`, and added harness-ready rows in `docs/interop-matrix.md`; still needs an actual previous release package run and checked-in pass evidence. |
| TG-007 | P1 | in_progress | Expand peer diversity beyond the current Samba 4.17.12 image. The Samba harness now accepts image/Dockerfile/peer-label inputs, and `eng/run-samba-peer-matrix.ps1` can run Bookworm and Trixie Debian-based Samba peers while preserving per-peer artifacts. | Complete when the additional peer run has passing evidence checked in or attached, `docs/interop-matrix.md` distinguishes each Samba target version or records a multi-version matrix, and artifacts include `smbd`, `smbclient`, OS, and Docker image identifiers. | Added `eng/docker/samba-interop/Dockerfile.trixie`, `eng/run-samba-peer-matrix.ps1`, and peer-label support in `eng/run-samba-interop.ps1`; still needs the additional peer evidence refresh. |
| TG-008 | P1 | in_progress | Add external DFS interoperability. `eng/run-dfs-interop.ps1` now targets a pre-provisioned external DFS namespace path, fetches referrals through the managed client, verifies an immediate second resolution comes from the referral cache, reconnects to the redirected storage target, and exercises directory-create, file-write, enumerate, read, rename, delete, and cleanup against that target while writing `artifacts/dfs-interop/dfs-interop.json`. `eng/run-integration-gates.ps1` now auto-invokes that lane when `OPENCIFS_DFS_*` environment variables are fully configured, so recurring external runs can include DFS namespace coverage without putting secrets on the command line. | `docs/coverage-matrix.md` no longer marks external DFS interop as backlog; `docs/interop-matrix.md` has a DFS row with artifact-backed pass evidence. | Added `eng/run-dfs-interop.ps1`, wired it into `eng/run-integration-gates.ps1`, documented the environment-driven path in `README.md`, and added harness-ready DFS notes in the matrices; still needs a provisioned Windows or Samba DFS namespace and passing evidence refresh. |
| TG-009 | P1 | in_progress | Add external durable-handle v2 and broader SMB 3.x behavior coverage. The external Python, Samba, and Windows harness artifacts now record structured `advanced_smb3` outcome data on SMB 3.x lanes, and the `OpenCIFS.Client -> Samba` console now attempts durable create, abrupt disconnect, durable reconnect, preserved byte-range lock behavior, exclusive oplock break, and lease break against Samba while the other external lanes record explicit skip reasons when their bounded harnesses do not surface reconnect tokens or break notifications. | Samba, Windows-client, and Python-client artifacts explicitly report advanced SMB 3.x durable-handle v2 or skip data, at least one external peer has checked-in pass evidence or a documented unsupported outcome for durable-handle v2, and the matrix notes no longer describe the entire area as an undifferentiated backlog item. | Updated `eng/real-client-smoke.py`, `eng/run-real-client-interop.ps1`, `eng/run-samba-interop.ps1`, `eng/run-windows-client-interop.ps1`, `eng/run-windows-client-interop-single.ps1`, and `src/OpenCIFS.SambaInterop.Console`; still needs checked-in external pass evidence refresh or documented unsupported outcomes from a live rerun. |
