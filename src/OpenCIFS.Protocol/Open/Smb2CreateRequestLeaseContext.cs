namespace OpenCIFS.Protocol
{
    ...
    /// <summary>
    /// SMB2 create request lease create-context helper for SMB 2.1.
    /// </summary>
    public sealed class Smb2CreateRequestLeaseContext
    {
        ...
        /// <summary>
        /// Reserved request flags. Must be zero for SMB 2.1.
        /// </summary>
        ...
        /// <summary>
        /// Determine whether a generic create context is an SMB 2.1 lease request context.
        /// </summary>
        ...
        /// <summary>
        /// Convert the typed lease request context to a generic SMB2 create context.
        /// </summary>
        ...
        /// <summary>
        /// Parse a typed SMB 2.1 lease request context from a generic create-context entry.
        /// </summary>
        /// <param name="context">Create-context entry.</param>
        /// <returns>Parsed lease request context.</returns>
        public static Smb2CreateRequestLeaseContext ReadFrom(Smb2CreateContext context)
        {
            if (!IsMatch(context))
            {
                throw new ProtocolValidationException("The SMB2 create context is not an SMB 2.1 lease request context.", nameof(context));
            }

            LittleEndianReader reader = new LittleEndianReader(context.Data);
            Smb2CreateRequestLeaseContext leaseContext = new Smb2CreateRequestLeaseContext
            {
                LeaseKey = reader.ReadBytes(16),
                LeaseState = (Smb2LeaseState)reader.ReadUInt32(),
                LeaseFlags = reader.ReadUInt32(),
        ...
        /// <summary>
        /// Validate a typed SMB 2.1 lease request context.
        /// </summary>
        /// <param name="context">Context to validate.</param>
        public static void Validate(Smb2CreateRequestLeaseContext context)
        {
            if (context == null)
            {
                throw new ProtocolValidationException("The SMB2 create request lease context cannot be null.", nameof(context));
            }

            if (context.LeaseKey.Length != 16)
            {
                throw new ProtocolValidationException("The SMB2 create request lease context must contain a 16-byte lease key.", nameof(context));
            }

            if ((context.LeaseState & ~(Smb2LeaseState.ReadCaching | Smb2LeaseState.HandleCaching | Smb2LeaseState.WriteCaching)) != 0)
            {
                throw new ProtocolValidationException("The SMB2 create request lease context contains unsupported lease-state bits.", nameof(context));
            }

            if (context.LeaseFlags != 0)
            {
                throw new ProtocolValidationException("The SMB2 create request lease context flags must be zero for the current SMB 2.1 slice.", nameof(context));
            }

            if (context.LeaseDuration != 0)
            {
                throw new ProtocolValidationException("The SMB2 create request lease duration must be zero for the current SMB 2.1 slice.", nameof(context));
            }
        }
