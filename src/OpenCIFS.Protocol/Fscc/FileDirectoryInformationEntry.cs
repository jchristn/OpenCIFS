namespace OpenCIFS.Protocol
{
    ...
    /// </summary>
    public sealed class FileDirectoryInformationEntry
    {
        private const int FixedBodyLength = 64;
        ...
        public static byte[] EncodeEntries(IReadOnlyList<FileDirectoryInformationEntry> entries)
        {
            if (entries == null)
            {
            ...
            {
                FileDirectoryInformationEntry entry = entries[index] ?? throw new ArgumentNullException(nameof(entries), "Entries cannot contain null values.");
        ...
        public static FileDirectoryInformationEntry[] DecodeEntries(ReadOnlyMemory<byte> buffer)
        {
            ...
            while (offset < buffer.Length)
            {
                ...
                {
                    throw new ProtocolEncodingException("The FILE_DIRECTORY_INFORMATION buffer does not contain a complete entry header.");
                }
                ...
                FileDirectoryInformationEntry entry = new FileDirectoryInformationEntry
                {
                ...
                {
                    throw new ProtocolEncodingException("The FILE_DIRECTORY_INFORMATION entry contains an invalid UTF-16 file-name length.");
                }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_DIRECTORY_INFORMATION entry name exceeds the available payload.");
                }
                ...
                if (nextEntryOffset == 0)
                {
                    ...
                    {
                        throw new ProtocolEncodingException("The FILE_DIRECTORY_INFORMATION buffer contains trailing bytes after the terminal entry.");
                    }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_DIRECTORY_INFORMATION entry NextEntryOffset is invalid.");
                }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_DIRECTORY_INFORMATION entry NextEntryOffset exceeds the available payload.");
                }
            }
