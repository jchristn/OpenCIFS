namespace OpenCIFS.Protocol
{
    ...
    /// <summary>
    /// FILE_ID_FULL_DIR_INFORMATION entry.
    /// </summary>
    public sealed class FileIdFullDirectoryInformationEntry
    {
        private const int FixedBodyLength = 80;
        ...
        public static byte[] EncodeEntries(IReadOnlyList<FileIdFullDirectoryInformationEntry> entries)
        {
            if (entries == null)
            {
            ...
            {
                FileIdFullDirectoryInformationEntry entry = entries[index] ?? throw new ArgumentNullException(nameof(entries), "Entries cannot contain null values.");
                ...
                writer.WriteUInt32(entry.Reserved);
        ...
        public static FileIdFullDirectoryInformationEntry[] DecodeEntries(ReadOnlyMemory<byte> buffer)
        {
            ...
            while (offset < buffer.Length)
            {
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_FULL_DIR_INFORMATION buffer does not contain a complete entry header.");
                }
                ...
                FileIdFullDirectoryInformationEntry entry = new FileIdFullDirectoryInformationEntry
                {
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_FULL_DIR_INFORMATION entry contains an invalid UTF-16 file-name length.");
                }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_FULL_DIR_INFORMATION entry name exceeds the available payload.");
                }
                ...
                if (nextEntryOffset == 0)
                {
                    ...
                    {
                        throw new ProtocolEncodingException("The FILE_ID_FULL_DIR_INFORMATION buffer contains trailing bytes after the terminal entry.");
                    }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_FULL_DIR_INFORMATION entry NextEntryOffset is invalid.");
                }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_FULL_DIR_INFORMATION entry NextEntryOffset exceeds the available payload.");
                }
            }
