namespace OpenCIFS.Protocol
{
    ...
    /// </summary>
    public sealed class FileIdBothDirectoryInformationEntry
    {
        private const int FixedBodyLength = 104;
        ...
        public static byte[] EncodeEntries(IReadOnlyList<FileIdBothDirectoryInformationEntry> entries)
        {
            if (entries == null)
            {
            ...
            {
                FileIdBothDirectoryInformationEntry entry = entries[index] ?? throw new ArgumentNullException(nameof(entries), "Entries cannot contain null values.");
        ...
        public static FileIdBothDirectoryInformationEntry[] DecodeEntries(ReadOnlyMemory<byte> buffer)
        {
            ...
            while (offset < buffer.Length)
            {
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_BOTH_DIR_INFORMATION buffer does not contain a complete entry header.");
                }
                ...
                FileIdBothDirectoryInformationEntry entry = new FileIdBothDirectoryInformationEntry
                {
                ...
                reader.ReadUInt16();
                entry.FileId = reader.ReadUInt64();

                if ((fileNameLength % 2) != 0 || (shortNameLength % 2) != 0 || shortNameLength > ShortNameFieldLength)
                {
                    throw new ProtocolEncodingException("The FILE_ID_BOTH_DIR_INFORMATION entry contains an invalid UTF-16 name length.");
                }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_BOTH_DIR_INFORMATION entry name exceeds the available payload.");
                }
                ...
                if (nextEntryOffset == 0)
                {
                    ...
                    {
                        throw new ProtocolEncodingException("The FILE_ID_BOTH_DIR_INFORMATION buffer contains trailing bytes after the terminal entry.");
                    }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_BOTH_DIR_INFORMATION entry NextEntryOffset is invalid.");
                }
                ...
                {
                    throw new ProtocolEncodingException("The FILE_ID_BOTH_DIR_INFORMATION entry NextEntryOffset exceeds the available payload.");
                }
            }
        ...
        private static byte[] EncodeShortName(string value)
        {
            ...
            {
                throw new ProtocolValidationException("The FILE_ID_BOTH_DIR_INFORMATION short name must fit within 24 bytes.", nameof(value));
            }
