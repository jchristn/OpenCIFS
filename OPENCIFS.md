...
- [-] Implement SMB1 session setup, tree connect, and logoff flows.
  Bounded progress: `Smb1SessionSetupAndXRequest` and `Smb1SessionSetupAndXResponse` codecs now carry the NT LM 0.12 extended-security shape (Unicode strings only) including the AndX header, MaxBufferSize / MaxMpxCount / VcNumber / SessionKey / Capabilities / SecurityBlob / NativeOS / NativeLanMan on the request side and AndX header / Action / SecurityBlob / NativeOS / NativeLanMan / PrimaryDomain on the response side, with proper Unicode 2-byte alignment padding before the trailing strings per MS-CIFS section 2.2.4.6.2. Shared core suite pins request and response round-trip plus guest-action-bit preservation. SMB1 TREE_CONNECT_ANDX, LOGOFF_ANDX, and the broader command-set codecs (NTCreateAndX, ReadAndX, WriteAndX, Close, LockingAndX, Echo, Transaction families) remain backlog.
