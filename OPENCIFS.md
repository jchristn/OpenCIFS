...
- [-] Expand Samba peer diversity beyond the current Bookworm image.
  Bounded progress: `eng/run-samba-interop.ps1` now accepts explicit Samba image, Dockerfile, and peer-label parameters, and `eng/run-samba-peer-matrix.ps1` can run the same interop flow against checked-in Bookworm and Trixie Debian-based Samba image definitions while preserving per-peer artifacts. The additional peer still needs a passing evidence refresh before `docs/interop-matrix.md` can claim multi-version Samba coverage.
- [-] Add Linux kernel CIFS client coverage for `Sample.OpenCifsServer`.
  Bounded progress: `eng/run-linux-cifs-interop.ps1` now builds a Dockerized `cifs-utils` client, starts `Sample.OpenCifsServer`, mounts the share through privileged `mount.cifs`, and drives SMB 2.0.2, SMB 2.1, and SMB 3.0.2 create/read/write/large-file/rename/delete cleanup flows. The harness still needs a privileged Docker host with CIFS mount support and checked-in pass evidence before the matrix can claim Linux kernel CIFS interoperability.
...
- [-] Implement `NTCreateAndX`, `ReadAndX`, `WriteAndX`, `Close`, `LockingAndX`, `Echo`, and related core file operation flows.
  Bounded progress: `Smb1NtCreateAndXRequest` and `Smb1NtCreateAndXResponse` codecs now carry the WordCount=24 request (AndX header, NameLength, Flags, RootDirectoryFileId, DesiredAccess, AllocationSize, ExtFileAttributes, ShareAccess, CreateDisposition, CreateOptions, ImpersonationLevel, SecurityFlags, Unicode-aligned FileName) and WordCount=34 response (AndX header, OplockLevel, FID, CreateDisposition, four NT FILETIME timestamps, ExtFileAttributes, AllocationSize, EndOfFile, ResourceType, NMPipeStatus, Directory) per MS-CIFS section 2.2.4.64 with the alignment-pad logic computed against the carrying header position. `Smb1ReadAndXRequest` and `Smb1ReadAndXResponse` codecs carry the WordCount=12 request (AndX header, FID, 64-bit FileOffset = Offset+OffsetHigh, MaxCountOfBytesToReturn, MinCountOfBytesToReturn, Timeout/MaxCountHigh, Remaining) and WordCount=12 response (AndX header, Available, DataCompactionMode, DataLength, DataOffset, DataLengthHigh, four reserved words, ByteCount-prefixed data block with 1-byte alignment pad) per MS-CIFS section 2.2.4.42. `Smb1WriteAndXRequest` and `Smb1WriteAndXResponse` codecs carry the WordCount=14 request (AndX header, FID, 64-bit FileOffset, Timeout/Reserved, WriteMode, Remaining, DataLengthHigh, DataLength, DataOffset, ByteCount-prefixed data block with 1-byte alignment pad) and WordCount=6 response (AndX header, Count, Available, CountHigh) per MS-CIFS section 2.2.4.43. `Smb1CloseRequest` and `Smb1CloseResponse` carry the WordCount=3 request (FID + LastWriteTime UTIME) and WordCount=0 response per MS-CIFS section 2.2.4.5, with the `LastWriteTimeUnchanged` (`0xFFFFFFFF`) sentinel surfaced for callers that want to preserve modification timestamps. `Smb1EchoRequest` and `Smb1EchoResponse` carry the WordCount=1 EchoCount/SequenceNumber plus variable-length ByteCount-prefixed buffer round-trip per MS-CIFS section 2.2.4.39. `Smb1LockingAndXRequest` and `Smb1LockingAndXResponse` codecs carry the WordCount=8 request (AndX header, FID, LockType, OplockLevel, Timeout, NumberOfUnlocks, NumberOfLocks, ByteCount-prefixed lock-range buffer with 20-byte large-file entries when LockType bit 0x10 is set or 10-byte legacy entries otherwise) and WordCount=2 response (AndX header only) per MS-CIFS section 2.2.4.32, with the public `LockTypeLargeFiles` and `LockTypeOplockRelease` constants surfaced for callers and a typed `LockRange` entry shape that round-trips ProcessId, 64-bit Offset, and 64-bit Length. Shared core suite pins all twelve round-trip paths plus non-Unicode rejection on NT_CREATE_ANDX, non-zero ByteCount rejection on CLOSE/NT_CREATE_ANDX response/READ_ANDX request/WRITE_ANDX response/LOCKING_ANDX response, ByteCount-mismatch rejection on ECHO, and ByteCount-vs-declared-lock-count mismatch rejection on LOCKING_ANDX. Transaction/Transaction2/NT_TRANSACT codecs and real client/server selection of all SMB1 file-I/O paths remain backlog.
- [-] Implement `Transaction`, `Transaction2`, and `NT_TRANSACT` families required for directory enumeration, query/set info, and filesystem info.
  Bounded progress: `Smb1Transaction2Request` and `Smb1Transaction2Response` codecs now carry the WordCount=15 primary request and WordCount=10 response shapes per MS-CIFS section 2.2.4.46. The request shape carries TotalParameterCount/TotalDataCount, MaxParameterCount/MaxDataCount, MaxSetupCount, Trans2 flags, Timeout, ParameterCount/ParameterOffset, DataCount/DataOffset, SetupCount=1 plus a single typed `Smb1Transaction2SubCommand` Setup word, and the ByteCount-prefixed (Name + Pad1 + Parameters + Pad2 + Data) payload with parameter and data blocks aligned to 4-byte boundaries from the SMB header start. The response shape mirrors the request with TotalParameterCount/TotalDataCount, ParameterCount/ParameterOffset/ParameterDisplacement, DataCount/DataOffset/DataDisplacement, SetupCount=0, and the same 4-byte-aligned payload layout. The new `Smb1Transaction2SubCommand` enum surfaces typed sub-command codes for Open2 / FindFirst2 / FindNext2 / QueryFsInformation / SetFsInformation / QueryPathInformation / SetPathInformation / QueryFileInformation / SetFileInformation / FsctlOrNotifyChange / CreateDirectory / SessionSetup / GetDfsReferral / ReportDfsInconsistency. Public `FlagDisconnectTid` (0x0001) and `FlagOneWay` (0x0002) constants are also surfaced for callers. Shared core suite pins request and response round-trip plus oversize-ByteCount rejection on the request and non-zero SetupCount rejection on the response. `Smb1TransactionRequest` codec now carries the WordCount=14+SetupCount Trans1 primary-request shape per MS-CIFS section 2.2.4.33.1 with TotalParameterCount/TotalDataCount, MaxParameterCount/MaxDataCount, MaxSetupCount, Trans1 flags, Timeout, ParameterCount/ParameterOffset, DataCount/DataOffset, the variable-count Setup-word block, plus a ByteCount-prefixed (PadBeforeName + Name + Pad1 + Parameters + Pad2 + Data) payload that supports both Unicode and ASCII transaction names with the canonical 2-byte alignment pad before Unicode names. `Smb1NtTransactRequest` codec now carries the WordCount=19+SetupCount NT_TRANSACT primary-request shape per MS-CIFS section 2.2.4.62.1 with 32-bit TotalParameterCount/TotalDataCount/MaxParameterCount/MaxDataCount/ParameterCount/ParameterOffset/DataCount/DataOffset, a 16-bit Function code identifying the NT_TRANSACT sub-command (NT_TRANSACT_CREATE = 1, NT_TRANSACT_IOCTL = 2, NT_TRANSACT_NOTIFY_CHANGE = 4, etc.), the variable-count Setup-word block, and the same 4-byte-aligned (Pad1 + Parameters + Pad2 + Data) payload layout. `Smb1TransactionResponse` codec now carries the WordCount=10+SetupCount Trans1 response shape with TotalParameterCount/TotalDataCount, ParameterCount/Offset/Displacement, DataCount/Offset/Displacement, the variable-count Setup-word block, and the same 4-byte-aligned ByteCount-prefixed payload. `Smb1NtTransactResponse` codec now carries the WordCount=18+SetupCount NT_TRANSACT response shape with 32-bit count/offset/displacement fields and a Reserved2 byte before the Setup-word block. `Smb1TransactionSecondary` (WordCount=8), `Smb1Transaction2Secondary` (WordCount=9 with trailing FID), and `Smb1NtTransactSecondary` (WordCount=18 with 32-bit fields and trailing Reserved2 byte) codecs carry the multi-fragment continuation shapes per MS-CIFS sections 2.2.4.34.1 / 2.2.4.47.1 / 2.2.4.63.1. The `Smb1Command` enum now exposes `TransactionSecondary` (`0x26`), `Transaction2Secondary` (`0x33`), and `NtTransactSecondary` (`0xA1`). Shared core suite pins all five new round-trip paths and all eight existing transaction-family round-trip paths. Real client/server selection of all SMB1 metadata paths, multi-fragment reassembly state machine, and the bound INFO-level structure codecs (TRANS2_QUERY_FILE_INFORMATION / TRANS2_SET_FILE_INFORMATION / TRANS2_FIND_FIRST2 / TRANS2_FIND_NEXT2) remain backlog.
- [x] Implement SMB1 error/status mappings and compatibility shims where semantics differ from SMB2/3.
  Verified scope: `Smb1DosErrorClass` enum carries the canonical `Success`/`ErrDos`/`ErrSrv`/`ErrHrd`/`ErrCmd` class bytes per MS-CIFS section 2.2.2.4; `Smb1DosErrorCode` static class surfaces the most commonly negotiated DOS error codes; and the new `Smb1StatusMapping` static class translates between modern `NtStatus` values and the legacy `(class, code)` pair through `NtStatusToDosError(...)` / `DosErrorToNtStatus(...)` plus the packed legacy 4-byte status-field helpers `PackNtStatusToLegacyStatusField(...)` / `UnpackLegacyStatusFieldToNtStatus(...)`. Modern OpenCIFS clients and servers continue to set the `Smb1HeaderFlags2.NtStatus` bit so the 4-byte status field carries an NTSTATUS directly; the mapping table exists for the bounded SMB1 dispatch slice that has to interoperate with pre-NT-aware peers and for surface diagnostics. Shared core suite pins forward and reverse mapping for the eighteen most common NTSTATUS values, generic-failure fallback for unknown NTSTATUS values, `STATUS_UNSUCCESSFUL` fallback for unknown DOS combos, and pack-then-unpack round-trip preservation. NTSTATUS additions to support the mapping table include `Unsuccessful`, `InvalidDeviceRequest`, `LogonFailure`, `DiskFull`, `InsufficientResources`, `NetworkNameDeleted`, `BadNetworkName`, `TooManyOpenedFiles`, and `UserSessionDeleted`.
- [-] Implement NetBIOS session service behavior required for SMB1 interoperability where applicable.
  Bounded progress: the existing 4-byte `NetBiosSessionServiceHeader` (Type + 24-bit BigEndian Length) is now joined by `NetBiosEncodedName` (RFC 1001 first-level encoding: 16-byte raw NetBIOS name split into 32 ASCII letters from A-P, with a 1-byte length marker `0x20` and a single-zero scope terminator for 34 bytes total, plus `FromServiceName(name, suffix)` factory and the public `FileServerServiceSuffix` (`0x20`) and `WorkstationServiceSuffix` (`0x00`) constants), `NetBiosSessionRequest` (4-byte header + 68-byte payload of called and calling encoded names per RFC 1002 section 4.3.2), and `NetBiosNegativeSessionResponse` (4-byte header + 1-byte error code per RFC 1002 section 4.3.4 with the typed `NetBiosNegativeSessionResponseErrorCode` enum). Shared core suite pins all three new round-trip paths plus non-`0x20` length-marker rejection, truncated-buffer rejection, mismatched message-type rejection, and non-1 declared-length rejection. Real port-139 listener wiring and SMB1-over-NetBIOS dispatch remain backlog.
...
- [-] Implement DFS referral request and response handling per the planned claim scope.
  Bounded progress: `OpenCIFS.Protocol` now carries `DfsReferralRequest`, `DfsReferralResponse`, `DfsReferralEntryV2`, `DfsReferralEntryV3`, `DfsReferralEntryFlags`, `DfsReferralRequestEx`, and `DfsReferralHeaderFlags` for the V2 path-consumer entry shape and the V3/V4 path-consumer entry shape (with the 16-byte ServiceSiteGuid and TargetSetBoundary V4 flag preservation), plus the SMB 3.0+ DFS_GET_REFERRALS_EX request payload that conveys the requesting client's site name. `DfsReferralResponse` now exposes typed `EntriesV2` / `EntriesV3` lists and dispatches per-entry decode by peeking the version field at each offset. Shared core suite pins V2 round-trip, V3/V4 round-trip with TargetSetBoundary preservation, EX-request round-trip with and without SiteName, and EX-request truncated-buffer rejection. The bounded slice also fixed an off-by-four `DfsReferralEntryV2` fixed-header length so referral string offsets land on the correct bytes. Broader NameList referral entry shape (used for DC referrals), namespace-root referrals, broader DFS namespace semantics, and external Windows or Samba DFS interop remain backlog.
...
- [-] Run parser mutation and malformed-input suites at scale.
  Bounded progress: `OpenCIFS.Core.Tests.Shared` now runs deterministic parser-mutation corpuses across 23 readers spanning direct-TCP, SMB2 base header, SMB2 negotiate request, SMB2 create request, SMB2 change-notify, SMB2 IOCTL, SMB2 lease-break ack, SMB2 query-info, SMB2 file-both-directory information, SMB2 compound packets, SMB 3.1.1 negotiate-context list, DFS V2 request and response, DFS V3/V4 response, DFS_GET_REFERRALS_EX request, NetBIOS SESSION_REQUEST, and the full SMB1 codec foundation (negotiate response, session-setup, tree-connect, NT_CREATE_ANDX, READ_ANDX, WRITE_ANDX, LOCKING_ANDX, TRANSACTION2 primary request, NT_TRANSACT primary request). Each baseline yields a deterministic corpus of approximately 64-160 mutated payloads through XOR/zero/shift/random/truncation strategies; the full corpus now executes at least 1500 mutated payloads with at least 600 rejected as malformed and continues to reject without unexpected parser failures. `OpenCIFS.Server.Tests.Shared` runs a malformed direct-TCP mutation burst against the live listener while verifying post-burst negotiate recovery and bounded protocol-exception surfacing. Longer-running mutation volume past the deterministic corpus and broader client/interop malformed-input scaling remain backlog.
