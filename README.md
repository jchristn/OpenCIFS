...
- `eng/run-nightly-interop.ps1` now provides a deeper current-dialect interop harness: it reruns the Python real-client, Samba, and native Windows three-dialect matrices across SMB 2.0.2, SMB 2.1, and encryption-required SMB 3.0.2 with a larger bounded payload and then composes that with a stronger SMB 2.1 soak for durable reconnect, exclusive oplock-break, lease-break, and large-I/O churn. The resulting nightly artifact is written to `artifacts/nightly-interop/nightly-interop.json`. The external per-peer artifacts now also record structured advanced SMB 3.x durable/oplock/lease outcome or skip data, while checked-in durable-handle v2 pass evidence, SMB 3.1.1 negotiation, and broader SMB 3.x nightly evidence remain backlog.
- The consolidated `OpenCIFS.Test.Shared` core and server suites now include deterministic parser-mutation coverage for representative frame and request readers plus malformed direct-TCP listener bursts, and the current managed-path listener normalizes malformed top-level request traffic into bounded protocol exceptions without taking down later healthy negotiate flows.
...
To exercise `OpenCIFS.Client` against a real external DFS namespace, provision a writable same-credential DFS path first, then run:

```powershell
powershell -ExecutionPolicy Bypass -File .\eng\run-dfs-interop.ps1 -ServerName dfsserver -ShareName namespace -NamespacePath links\public -UserName user -Password password -Domain DOMAIN
```

That script fetches DFS referrals, verifies that a second resolution comes from the client referral cache, follows the redirected storage target, and then creates, reads, renames, and deletes a bounded file flow beneath that target while recording `artifacts/dfs-interop/dfs-interop.json`. It also accepts the same settings through `OPENCIFS_DFS_SERVER_NAME`, `OPENCIFS_DFS_SHARE_NAME`, `OPENCIFS_DFS_NAMESPACE_PATH`, `OPENCIFS_DFS_USER_NAME`, `OPENCIFS_DFS_PASSWORD`, and the optional `OPENCIFS_DFS_DOMAIN`, `OPENCIFS_DFS_PORT`, or `OPENCIFS_DFS_PEER_LABEL` environment variables. The current harness assumes the namespace and redirected storage target use the same credentials and TCP port.
